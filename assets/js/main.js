...
// automatically update navigation based on scroll position to indicate the active link
    ...
    <div class="col-sm-6 col-md-4 col-lg-3 my-3">
      <div class="card text-center shadow">
        ...
        <div class="card-body">
          ...
          <p class="text-center card-text price"><strong>        <span>${displayPrice(
            item.price,
            "int"
          )}</span>
        <sup>${displayPrice(item.price, "dec")}</sup></strong><span>lei/${
    item.unit
  }</span></p>
          <button class="btn btn-primary add-cart" type="button" onclick="addToCart(${
            item.id
          }, ${index})"><i class="fa fa-shopping-cart"></i>Adaugă în coș</button>
        </div>
      </div>
    </div>
    `);
...
function updateCartDisplay() {
  ...
  <div class="d-flex justify-content-between cart-item">
    <div class="cart-img"><img class="img-fluid" src="assets/img/${
      item.img
    }" /></div>
    <div class="d-flex flex-column justify-content-center cart-info">
        <h6 class="cart-item-name">${item.name}</h6>
        <p>1 <span class="cart-product-unit">${
          item.unit
        }</span> x <strong><span class="cart-product-price"><span>${displayPrice(
      item.price,
      "int"
    )}</span>
 ...
 `);
    ...
    $("#table-items").append(`
       <tr>
          ...
          <td><img class="img-fluid" style="width:4rem" src="assets/img/${
            item.img
          }"></td>
          <td>${item.name}</td>
        ...
        <sup>${displayPrice(item.price, "dec")}</sup> Lei</td>
          <td>
            ...
            <input style="width:2rem;text-align:center" id="${
              item.id
 ...
 `);
  ...
  $("#mini-cart-total-price").html(`<span>${displayPrice(
    totalMiniPrice,
    "int"
  )}</span>
...
function displayPrice(num, part) {
  if (num) {
    ...
    switch (part) {
      ...
      default:
        console.log(
          'First argument must be a number and the second argument must be one of the following strings: "int" or "dec".'
        );
    }
  }
}
...
$("#newsletter-submit").click(function () {
  ...
  if (
    $("#newsletter-name").get(0).reportValidity() &&
    $("#newsletter-email").get(0).reportValidity()
  ) {
    firebase
      .database()
      .ref("newsletter")
    ...
    Swal.fire({
      ...
      text:
        "Am inregistrat adresa ta de email, de acum inainte vei primi periodic informatii importante de la noi.",
    });
  }
});
...
$("#contact-submit").click(function () {
  event.preventDefault();
  if (
    $("#contact-name").get(0).reportValidity() &&
    $("#contact-email").get(0).reportValidity() &&
    $("#contact-message").get(0).reportValidity()
  ) {
    firebase
      .database()
      .ref("messages")
    ...
    Swal.fire({
      ...
      text:
        "Suntem bucurosi sa primi sugestii de orice fel din partea clientilor nostri.",
    });
  }
});
...
$("#order-submit").click(function () {
  event.preventDefault();
  if (
    $("#order-first-name").get(0).reportValidity() &&
    $("#order-last-name").get(0).reportValidity() &&
    $("#order-address").get(0).reportValidity() &&
    $("#order-phone").get(0).reportValidity() &&
    $(".form-check-input").get(0).reportValidity()
  ) {
    ...
    Swal.fire({
      ...
      text:
        "In cel mai scurt timp te vom suna pentru detalii legate de plata si transport.",
    });
  }
});
